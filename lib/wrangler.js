...
function loadConfig(path, env) {
    ...
    if (env && config.env && config.env[env]) {
        ...
        delete config['env'];
        lodash_merge_1(config, env_config);
    }
    else {
        // Use base env
        delete config['env'];
    ...
    if (config.vars) {
        for (var _i = 0, _a = Object.entries(config.vars); _i < _a.length; _i++) {
            ...
            if (typeof v === 'string')
                config.vars[k] = v.replace(placeholder, function (match, path) { return lodash_get_1(config, path, match); });
        }
    }
    ...
    if (config.secrets && config.vars && typeof config.vars['IDP'] === 'string') {
        ...
        for (var _c = 0, _d = Object.entries(config.secrets).filter(function (_a) {
            ...
            switch (typeof secret) {
                case 'string':
                    config.secrets[k] = secret.replace(placeholder, function (match, path) { return lodash_get_1(config, path, match); });
                    break;
                case 'object':
                    if (Array.isArray(secret)) {
                        config.secrets[k] = secret.map(function (v) {
                            return typeof v === 'string' ?
                                v.replace(placeholder, function (match, path) { return lodash_get_1(config, path, match); })
                                : v;
                        });
                    }
                    else {
                        for (var _f = 0, _g = Object.entries(secret); _f < _g.length; _f++) {
                            ...
                            if (typeof v === 'string')
                                secret[k_1] = v.replace(placeholder, function (match, path) { return lodash_get_1(config, path, match); });
                        }
                    }
            }
        }
